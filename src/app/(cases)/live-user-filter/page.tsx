...
export default function LiveUserFilterPage() {
  ...
  const userListUI = filteredUsers.map(user => <LiUser key={user.login.uuid} user={user} />)

  return (
    <section className='flex h-screen w-screen items-center justify-center'>
      <div className='w-[300px] overflow-hidden rounded-xl shadow-lg shadow-green-600/50 select-none'>
        {/* header start */}
        <Header handleSearch={handleSearch} inputDisabled={loading} />
        {/* header end */}
        {/* 用户列表start */}
        <ul className='h-[400px] overflow-y-auto bg-white py-3.5 text-black'>
          {loading ? <KineticLoading size={30} triangleAColor='#52b1c7' /> : userListUI}
        </ul>
        {/* 用户列表end */}
      </div>
    </section>
